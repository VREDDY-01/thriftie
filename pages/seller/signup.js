...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (password == confPassword) {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/seller/sellerSignup`, {
        method: "POST",
        headers: {
      ...
      if (res.status == "201") {
        toast.success("Seller Account Created Sucessfully! Please Login Again.", {
          position: "top-right",
          autoClose: 2000,
  ...
  return (
    ...
    <ToastContainer
      ...
      <div className="relative flex w-96 flex-col space-y-5 rounded-lg border bg-white px-5 py-10 shadow-xl sm:mx-auto">
        <div className="-z-10 absolute top-4 left-1/2 h-full w-5/6 -translate-x-1/2 rounded-lg bg-black sm:-right-10 sm:top-auto sm:left-auto sm:w-full sm:translate-x-0"></div>
        <div className="mx-auto mb-2 space-y-3">
          <h1 className="text-center text-3xl font-bold text-gray-700">
            Sign up as a Seller
          </h1>
          <p className="text-gray-500 text-center">Sign up to create an account</p>
        </div>
        ...
        <div className="flex w-full items-center">
          <button
            type="submit"
            className="shrink-0 inline-block w-36 rounded-lg bg-black py-3 font-bold text-white"
          >
            Sign up
          </button>
          <Link
